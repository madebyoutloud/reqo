...
The library provides four lifecycle hooks, described by the `HookList` interface:
...
::field-group
  ...
  ::field{name="request" type="Hook<[config: RequestConfig, state: RequestState], void>"}
...
client.on('error', (error) => {
  console.error(`[${error.config.id}] Failed: ${error.status} ${error.message}`)
})
```
...
client.on('error', (error) => {
  timings.delete(error.config.url)
})
```
