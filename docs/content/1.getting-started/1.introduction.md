...
- **OpenAPI Schemas** - Infer paths, parameters, bodies and responses from generated schema types
...
- **URL Building** - Automatic URL construction with path and query parameters
...
const users = await reqo.$get('https://api.example.com/users', {
  query: { limit: 10 }
})
